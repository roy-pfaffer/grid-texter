class Commander
  ...
  def do
    puts "<<< attempting to run command #{parser.sent_command}"
    send("user_#{parser.sent_command}", *(parser.sent_args))
  end

  def method_missing(selector)
    puts "<<< COULD NOT FIND COMMAND #{selector}"
  ...
  def user_help(*args)
    ...
    puts "<<< USER_HELP COMMAND CALLED by #{registered_user[:alias]}"
    if (args.empty?)
      ...
      puts "<<< attempting to find help for #{args}"
      sender.send_reply("#{args.first}: #{command_description[args.first]}")
    end
  end

  def user_alias(new_alias)
    puts "<<< USER_ALIAS COMMAND CALLED by #{registered_user[:alias]}"
  ...
  def user_invite(*args)
    puts "<<< USER_INVITE COMMAND CALLED by #{registered_user[:alias]}"
  ...
  def user_leave
    puts "<<< USER_LEAVE COMMAND CALLED by #{registered_user[:alias]}"
  ...
  def user_roster
    puts "<<< USER_ROSTER COMMAND CALLED by #{registered_user[:alias]}"
    everyone_else = User.exclude(:sms_number => parser.from_number.to_i).map(:alias).join(', ')
    puts "<<< #{everyone_else} currently registered.  #{everyone_else.empty?}"
  ...
  def user_whoshere
    puts "<<< USER_WHOSHERE COMMAND CALLED by #{registered_user[:alias]}"
    everyone_else = User.where(:active => true).exclude(:sms_number => parser.from_number.to_i).map(:alias).join(', ')
    puts "<<< #{everyone_else} currently in the channel.  #{everyone_else.empty?}"
    (!everyone_else.empty?) ? sender.send_reply(everyone_else) : sender.send_reply("You're alone.  Invite someone to join the conversation with the /invite command.")
  end

  def user_unsubscribe
    puts "<<< USER_UNSUBSCRIBE COMMAND CALLED by #{registered_user[:alias]}"
